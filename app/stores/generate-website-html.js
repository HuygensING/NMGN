					...
					siteJson.forEach(chapterData => {
						if (chapterData !== null) {
							fs.readFile('./output/json/d'+chapterData.part+'h'+chapterData.chapter+'.json', 'utf8', (err, data) => {
								...
								utility.createFile('./output/html-site/d'+chapterJson.chapterMetadata.part+'h'+chapterJson.chapterMetadata.chapter+'-'+utility.saveTitle(chapterJson.chapterMetadata.title)+'.html', outputHTML)
							});
						}
...
// function renderNavBasic(indexJson) {
// 	let renderOut=''
// 	indexJson.forEach(navItem => {
// 		//console.log(navItem);
// 		// renderOut += '<a href="'+utility.saveTitle(navItem.title)+'.html'+'">'
// 		// renderOut += navItem.title
// 		// renderOut += '</a>'
// 	});
